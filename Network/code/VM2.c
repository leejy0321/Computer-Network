...
// 받은 패킷의 ip 헤더를 통해 해당 패킷의 목적지를 확인하고
// 나의 라우팅 엔트리에 일치하는 목적지가 있는지 확인 후
// 있다면 해당 엔트리를 반환하고 없다면 NULL 반환
...
void environment_setup() {
	...
	sscanf("08:00:27:e2:39:e3", "%hhx:%hhx:%hhx:%hhx:%hhx:%hhx", &mac[0], &mac[1], &mac[2], &mac[3], &mac[4], &mac[5]);
	add_arp_cache_entry("enp0s3", "Dongdaemungu 1", mac);
	sscanf("08:00:27:24:0f:3b", "%hhx:%hhx:%hhx:%hhx:%hhx:%hhx", &mac[0], &mac[1], &mac[2], &mac[3], &mac[4], &mac[5]);
